		...
		event == FL_DRAG)
	{
		...
		if(raytracer) 
		{
			...
			if(!raytracer->isReady()) {
				bool m_nEnableBVH = traceUI->enableBVHEnabled();
				bool m_nEnableAntialiasing = traceUI->enableAntialiasingEnabled();
				bool m_nEnableGlossyReflection = traceUI->enableGlossyReflectionEnabled();
				raytracer->traceSetup(m_nWindowWidth, m_nWindowHeight, m_nEnableBVH, m_nEnableAntialiasing, m_nEnableGlossyReflection);
			}
